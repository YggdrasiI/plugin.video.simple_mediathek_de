...
// Modification with extra commands before QSORT_LT is called.
/* The main code starts here... */
#define QSORT_WITH_PREPARE(QSORT_TYPE,QSORT_BASE,QSORT_NELT,QSORT_LT,PREPARE_LT)		\
{									\
  QSORT_TYPE *const _base = (QSORT_BASE);				\
  const unsigned _elems = (QSORT_NELT);					\
  QSORT_TYPE _hold;							\
  ...
  if (_elems > _QSORT_MAX_THRESH) {					\
    ...
    while (_QSORT_STACK_NOT_EMPTY) {					\
      ...
      /* Here's the famous ``collapse the walls'' section of quicksort.	\
         ...
         that this algorithm runs much faster than others. */		\
      do {								\
				PREPARE_LT (_left_ptr, _mid); \
        while (QSORT_LT (_left_ptr, _mid)){				\
					++_left_ptr;							\
					PREPARE_LT (_left_ptr, _mid);} \
									\
				PREPARE_LT (_mid, _right_ptr); \
        while (QSORT_LT (_mid, _right_ptr)){				\
					--_right_ptr;							\
					PREPARE_LT (_mid, _right_ptr);} \
									\
