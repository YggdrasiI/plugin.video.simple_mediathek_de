...
class Dict(dict):
    ...
    def __setitem__(self, item, _value):
        # Assume that _value has no parent_Dict...
        if self.__contains__(item):
            self.changed = (_value != self.__getitem__(item))
        else:
            self.changed = True

        super(Dict, self).__setitem__(item, _value)
        """
        if isinstance(_value, Dict):
            super(Dict, self).__setitem__(item, _value)
            _value.parent_Dict = self
            self.set_changed()
...
class SimpleMediathek:
    ...
    def __init__(self, state=None):
        ...
        self.mode = None
        self.prev_mode = u"None"
        self.same_folder = False
        self.b_mvweb = False
        self.b_livestream = False
        self.addon_handle = "addon_handle_undefined"
    ...
    def update_db(self, bForceFullUpdate=False):
        largs = [u"update"]
        diff = self.try_diff_update() and not bForceFullUpdate
        if diff:
            largs.append(u"diff")
    ...
    def handle_update_side_effects(self, args):
        """
    ...
    def listing_add_list_names(self, listing, state, item, names,
                               i_selected=-1):
    ...
    def listing_add_dict_names(self, listing, state, item, names,
                               v_selected=None):
    ...
    def listing_add_min_max_entries(self, listing, state, mode,
                                    i_selected=-1):
        for i in [0, 1]:
    ...
    def listing_direction_toggle(self, listing, state, mode):
        i_other = 1 - self.get_current_pattern().get(u"iduration_dir", 0)
    ...
    def listing_add_calendar_entry(self, listing, state, b_selected=False):
    ...
    def listing_add_history(self, listing, state):
        history = self.get_history()
        for h in history:
            name = h.get(u"name", self.sprint_search_pattern(h))
    ...
    def listing_add_remove_entry(self, listing, state, item):
    ...
    def listing_add_back_entry(self, listing, state):
    ...
    def listing_add_search(self, listing, state):
        pattern = self.get_current_pattern()
    ...
    def listing_add_test(self, listing, state):
    ...
    def listing_add_livestreams(self, listing, state):
    ...
    def listing_add_search_results(self, listing, state, results):
        """ Example line:
        ...
        for f in results:  # .get(u"found", []):
            ...
            is_folder = (int(addon.getSetting(u"video_quality")) == 0)
            listing.append((url, li, is_folder))
            i += 1
    ...
    def listing_search_page_link(self, listing, state, results, ipage):
        if ipage < 0:
            ipage = -ipage - 1
        ...
        url = build_url(state, self.mode)
        pattern = self.get_current_pattern()
        if is_searchable(pattern):
            url = build_url({u"mode": (u"start_search_livestreams" if
                                       self.b_livestream else u"start_search"),
                             u"page": ipage}, state)
            li = xbmcgui.ListItem(name, iconImage=u"DefaultFolder.png")
            li.setProperty(u"IsPlayable", u"true")
            is_folder = True
    ...
    def gen_search_categories(self):
        pattern = self.get_current_pattern()
        last_update = self.get_last_update_time()
        allow_update = self.is_index_outdated()
        if last_update[0] == -1:
            update_str = u""
        else:
            update_str = u"%s %s %s" % (
        ...
        categories.append({u"name": getLocalizedString(32341), # u"Sender",
                           u"selection": self.get_pattern_channel(
                               pattern).upper(),
        ...
        categories.append({u"name": getLocalizedString(32342),  # u"Titel",
                           u"selection": self.get_pattern_title(pattern),
        ...
        if not self.b_mvweb:
            ...
            categories.append({u"name": getLocalizedString(32346),  # u"Beschreibung",
                               u"selection": self.get_pattern_desc(pattern),
    ...
    def main(self, argv):
        # RunScript handling
        if argv[1] == u"update_db":
            self.mode = u"background_script_call"
            xbmcgui.Dialog().notification(
            ...
            ok, tdiff = False, 0
            try:
                (ok, start, end, _) = mediathek.update_db(bForceFullUpdate=True)
        ...
        elif argv[1] == u"delete_local_data":
            self.mode = u"background_script_call"
            try:
                (path, name) = get_history_file_path()
                rm_files = []
                for _, _, files in os.walk(path):  # root, dirs, files
                    for f in files:
                        ext = f[f.rfind("."):]
            ...
            self.mode = u"not used"
        else:
            self.addon_handle = int(argv[1])
            xbmcplugin.setContent(self.addon_handle, u"files")  # Some skins require it
            ...
            # url_args = urlparse.parse_qs(argv[2][1:])
            url_args = unpack_url(argv[2][1:])
            self.mode = url_args.get(u"mode", None)
            self.prev_mode = url_args.get(u"prev_mode", u"None")  # str
            self.same_folder = (self.prev_mode != u"None")  # do NOT compare with 'is not'
            self.b_livestream = False
            self.b_mvweb = (addon.getSetting(u"use_mediathekviewweb") == u"true")

            # Update with unsaved changes, but not force
            # write of changes at end of script
            global expanded_state
            if save_state_in_url:
                expanded_state = url_args.get(u"state", {})
            else:
                expanded_state = load_volatile_state()

            self.update_state(expanded_state, False)

        # Handle modes
        if self.mode == u"update_db_over_gui":
            ok, tdiff = False, 0
            (ok, start, end, _) = self.update_db(
        ...
        if self.mode == u"select_calendar":
            # Not implemented
            ...
            if len(result1) > 0:
                ...
                try:
                    ...
                    self.update_state(changes, False)  # Unwritten update
                ...
                # except (ValueError, TypeError) as e:
                    ...
                    xbmcgui.Dialog().notification(
                        ...
                        xbmcgui.NOTIFICATION_ERROR, 5000)
                    self.mode = u"select_day"

        if self.mode == u"select_day":
            names = search_ranges_str[u"day"]
            i_sel = self.get_current_pattern().get(u"iday", -1)
            b_day_range = u"iday_range" in self.get_current_pattern()
            listing = []
            self.listing_add_list_names(listing, expanded_state,
                                        u"iday", names, i_sel)
            self.listing_add_calendar_entry(listing, expanded_state,
                                            b_day_range)
            self.listing_add_remove_entry(listing, self.state,
                                          "iday_range" if b_day_range else u"iday")
            self.listing_add_back_entry(listing, expanded_state)
            xbmcplugin.addDirectoryItems(self.addon_handle, listing,
                                         len(listing))
            xbmcplugin.endOfDirectory(
                self.addon_handle, updateListing=self.same_folder,
                cacheToDisc=directory_cache)

        if self.mode == u"select_time":
            names = search_ranges_str[u"time"]
            i_sel = self.get_current_pattern().get(u"itime", -1)
            listing = []
            self.listing_add_list_names(listing, expanded_state,
                                        u"itime", names, i_sel)
            self.listing_add_remove_entry(listing, self.state, u"itime")
            self.listing_add_back_entry(listing, expanded_state)
            xbmcplugin.addDirectoryItems(self.addon_handle, listing,
                                         len(listing))
            xbmcplugin.endOfDirectory(
        ...
        if self.mode == u"select_duration_dir":
            listing = []
            i_sel = self.get_current_pattern().get(u"iduration_dir", -1)
            self.listing_add_min_max_entries(listing, expanded_state,
        ...
        if self.mode == u"select_duration":
            # 0. Handle return value of select_duration_dir step
            if u"dir" in url_args:
                pattern = self.get_current_pattern()
                pattern.update({u"iduration_dir": url_args[u"dir"]})
                changes = {u"current_pattern": pattern}
                self.update_state(changes, False)  # Unwritten update
                expanded_state.update(changes)  # propagated by Uri
            ...
            i_sel = self.get_current_pattern().get(u"iduration", -1)
            listing = []
            self.listing_add_list_names(listing, expanded_state,
                                        u"iduration", names, i_sel)
            self.listing_add_remove_entry(listing, self.state, u"iduration")

            # 0b. Add toggle for direction flag
            if not duration_separate_minmax:
                self.listing_direction_toggle(listing, expanded_state,
                                              u"select_duration")
        ...
        if self.mode == u"select_channel":
            channels = self.get_channel_list()
            current_pattern = self.get_current_pattern()
            listing = []
            self.listing_add_dict_names(listing, expanded_state,
                                        u"channel", channels,
                                        current_pattern.get(u"channel", u""))
            self.listing_add_remove_entry(listing, self.state, u"channel")
            self.listing_add_back_entry(listing, expanded_state)
            xbmcplugin.addDirectoryItems(self.addon_handle, listing,
                                         len(listing))
            xbmcplugin.endOfDirectory(
        ...
        if self.mode == u"select_history":
            if u"pattern" in url_args:
                new_pattern = url_args.get(u"pattern")
                ...
                self.update_state(changes, True)  # Write update
                expanded_state.update(changes)

            # Value applied. Go back and show main menu
            self.mode = u"main"

        if self.mode in [u"select_title", u"select_desc"]:
            ...
            def _get_string(key, header, pattern, start_text):
                _result, query = keyboard.keyboard_input(header, start_text)
                if _result:
                    pattern[key] = query
                    changes = {u"current_pattern": pattern}
                    self.update_state(changes, False)  # Unwritten update
            ...
            arg_id = int(url_args.get(u"id", 0))
            if arg_id == last:
                self.mode = u"main"
            else:
                pattern = self.get_current_pattern()
                if self.mode == u"select_desc":
                    _get_string(u"description",  # u"Beschreibung")
                                getLocalizedString(32346),
                                pattern,
                                self.get_pattern_desc(pattern))
                else:
                    _get_string(u"title",  # u"Titel / Thema (No RegEx)")
                                getLocalizedString(32366),
                                pattern,
                                self.get_pattern_title(pattern))

                expanded_state[u"input_request_id"] = arg_id
                self.mode = u"main"

        if self.mode == u"select_result":
            results = self.get_search_results().get(u"found", [])

            if addon.getSetting(u"early_history_update") != u"true":
                pattern = self.get_current_pattern()
                self.add_to_history(pattern)

            try:
                result = results[url_args.get(u"iresult", -1)]
                ...
                if result.get(u"payload"):
                    ...
                    bin_args = [u"--payload", s_anchor]
                    (exit_code, data) = call_binary(bin_args)
                    if exit_code == 0:
                        js = json.loads(data)
                ...
                if len(non_empty_urls) == 0:
                    ...
                    if quali == 0:
                        # Show list
                        list_urls(self.addon_handle, expanded_state, urls,
                                  result, self.same_folder)
                    elif quali in [1, 2]:
                        url = non_empty_urls[1-quali]  # [0] or [-1]
                        # Play file
                        play_url(self.addon_handle, expanded_state, url, result)
                    else:
                        ...
                        if len(non_empty2) > 0:
                            # Play file
                            play_url(self.addon_handle, expanded_state, non_empty2[0], result)
                        else:
                            # Show list
                            list_urls(self.addon_handle, expanded_state, urls,
                                      result, self.same_folder)
        ...
        if self.mode == u"update_pattern":
            if u"item" in url_args:
                pattern = self.get_current_pattern()
                if u"value" in url_args:
                    try:
                        value = int(url_args[u"value"])
                        self.handle_update_side_effects(url_args)
                    except ValueError:
                        value = url_args[u"value"]

                    pattern.update({url_args[u"item"]: value})
                elif url_args[u"item"] in pattern:
                    pattern.pop(url_args[u"item"])
        ...
        if self.mode in [u"start_search", u"start_search_livestreams"]:
            if self.mode == u"start_search_livestreams":
                pattern = self.get_livestream_pattern()
                self.b_livestream = True
            else:
                pattern = self.get_current_pattern()
            ...
            if self.b_mvweb:
                ...
                page = u"%u,%u" % (max_num_entries_per_page + 1,
                                   url_args.get(u"page", 0)*max_num_entries_per_page)
            ...
            self.mode = u"show_search_result"

        if self.mode == u"show_search_result":
            if u"latest_search" in expanded_state:
                results = expanded_state[u"latest_search"].get(u"found", [])
                listing = []
                prev_page = url_args.get(u"page", 0) - 1
                next_page = (prev_page + 2) if (
                    len(results) > max_num_entries_per_page) else -1

                if prev_page > -1:
                    self.listing_search_page_link(listing, expanded_state,
                                                  results, -prev_page-1)

                self.listing_add_search_results(listing, expanded_state,
                                                results)
                if next_page > -1:
                    self.listing_search_page_link(listing, expanded_state,
                                                  results, next_page)
                ...
                if False:
                    # Had strange effects...
                    xbmcplugin.endOfDirectory(
                        self.addon_handle, updateListing=(prev_page >= 0),
                        cacheToDisc=directory_cache)
                else:
                    xbmcplugin.endOfDirectory(
                        self.addon_handle, updateListing=self.same_folder,
        ...
        if self.mode == u"show_history":
            listing = []
            self.listing_add_history(listing, expanded_state)
            self.listing_add_back_entry(listing, expanded_state)
            xbmcplugin.addDirectoryItems(self.addon_handle, listing,
                                         len(listing))
            xbmcplugin.endOfDirectory(self.addon_handle,
                                      updateListing=self.same_folder)

        if self.mode == u"play_url":
            if u"video_url" in url_args:
                play_url(self.addon_handle, expanded_state, url_args[u"video_url"])
        ...
        if self.mode is None or self.mode == u"main":
            ...
            search_categories = self.gen_search_categories()

            if self.mode is None:
                if self.b_mvweb:
                    if self.is_mvw_outdated():
                        self.update_mvw()
                        expanded_state[u"ilast_mvw_update"] = int(time.time())
                elif self.is_index_outdated():
            ...
            # self.listing_add_test(listing, expanded_state)
            self.listing_add_livestreams(listing, expanded_state)

            self.listing_add_search(listing, expanded_state)
            xbmcplugin.addDirectoryItems(self.addon_handle, listing,
                                         len(listing))
            xbmcplugin.endOfDirectory(
                self.addon_handle, updateListing=self.same_folder,
...
def build_url(query, mode, state=None):
...
def call_binary(largs):
    ...
    (path, _) = get_history_file_path()
    largs.append(u"--folder")
    largs.append(path)

    if addon.getSetting(u"pause_kodi_process") == u"true":
        largs.append("--paused")
    ...
    try:
        ...
        if addon.getSetting(u"debug_binary") == u"true":
            import random
...
def check_addon_status(b_mvweb):
...
def list_urls(addon_handle, state, urls, search_result, same_folder):
    """
...
with SimpleMediathek() as simple_mediathek:
    simple_mediathek.main(sys.argv)
